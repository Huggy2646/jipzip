...
public class BuildingMapperTest extends AbstractTest {
...
//		searchCondition3 = new SearchCondition("sido", "gugun", "325235620", 2015, 1);
	...
	public void searchTest() { // 검색 테스트
		// 개수 테스트 1
		List<Building> buildings1 = buildingMapper.search(searchCondition1);
		assertEquals(6, buildings1.size());
		// 개수 테스트 2
		List<Building> buildings2 = buildingMapper.search(searchCondition2);
		assertEquals(4, buildings2.size());
		// 개수 테스트 3
		List<Building> buildings3 = buildingMapper.search(searchCondition3);
